		...
		return {
			...
			link: function (scope, elem, attrs, controller) {
				...
				var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
					...
					dropdownTemplate =
                    '<input id="search-boxNew" class="search-input" data-toggle="dropdown" href ng-class="{ disabled: disabled }" placeholder="Search..." name="qq" />' +
					
                        '<label for="search-boxNew">' +
				...
				var dropdownElement = angular.element(dropdownTemplate),
					...
					inputElement = dropdownElement.eq(2).find(':text'),
					ulElement = dropdownElement.eq(3).find('ul');
